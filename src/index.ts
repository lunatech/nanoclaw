...
async function runAgent(
  ...
  prompt: string,
  chatJid: string,
  attachments: import('./types.js').ClaudeAttachment[] = [],
...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(
    `NanoClaw running (git sha: ${GIT_SHA}, default trigger: ${DEFAULT_TRIGGER})`,
  );
...
async function main(): Promise<void> {
  ...
  const customServices: Array<{ close(): Promise<void> }> = [];

  // Graceful shutdown handlers
  const shutdown = async (signal: string) => {
    logger.info({ signal }, 'Shutdown signal received');
    await stopCustomServices(customServices);
    proxyServer.close();
  ...
  const channelOpts = {
    onMessage: (chatJid: string, msg: NewMessage) => {
      // Sender allowlist drop mode: discard messages from denied senders first,
      // including privileged remote-control commands.
