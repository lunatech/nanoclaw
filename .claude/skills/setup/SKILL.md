...
- Docker installs: check `.env` for `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`
- OneCLI installs: check `onecli secrets list`
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common: wrong Node path (re-run step 7), missing or invalid credentials (check `.env` for direct auth or `onecli secrets list` plus `curl http://127.0.0.1:10254/api/health` for OneCLI), missing channel credentials (re-invoke channel skill).
